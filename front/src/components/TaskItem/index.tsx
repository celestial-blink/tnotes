...
const TaskItem = ({ id, title, isComplete, createdAt, endDate, onClickMenu, isDraft, onClickItem }: IProps) => {
    ...
    return (
        <div className="flex flex-wrap justify-between border rounded p-2 items-center bg-slate-100 dark:bg-slate-600 dark:text-white dark:border-none">
            <div className="flex gap-1 flex-1 h-full items-center justify-between">
                ...
                <div className="text-xs flex flex-col items-end flex-[.5]">
                    {!isDraft && isComplete ? <span className="block px-2 py-1 bg-green-400 rounded w-max dark:bg-green-500">Complete</span> : null}
                    {isDraft ? <span className="block px-2 py-1 bg-gray-300 rounded w-max dark:text-slate-800">Borradores</span> : null}
                    <span className="block text-slate-500 mt-1 whitespace-pre-wrap text-right dark:text-slate-200">{`Creado el ${createdAt} ${endDate ? `- Fecha limite ${endDate}` : ""}`}</span>
                </div>
            </div>
            <details className="details relative" ref={refDetails}>
                ...
                <menu className="as-menu absolute bg-white p-1 right-0 rounded w-max flex gap-2 flex-col dark:bg-slate-700" onClick={handleClickMenuItem}>
                    <li> <button data-action="update" className="p-1 w-full hover:bg-slate-200 text-right dark:hover:bg-slate-500">Editar</button> </li>
...
export const SkeletonTaskItem = () => <div className="py-6 bg-slate-100 rounded animate-pulse dark:bg-slate-600"></div>;
